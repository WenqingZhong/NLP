...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        bufflen=len(state.buffer)
        cdidx=self.word_vocab['<CD>']
        nnpidx=self.word_vocab['<NNP>']
        unkidx=self.word_vocab['<UNK>']
        rootidx=self.word_vocab['<ROOT>']
        nullidx=self.word_vocab['<NULL>']
        ...
        if(stacklen<3):
            ...
            for i in range(1,stacklen+1):                
                ...
                if (tag is None):
                    ...
                    result.append(nnpidx)
                elif(word in self.word_vocab):
                    
                    result.append(self.word_vocab[word])
                else:
                    
                    result.append(unkidx)
                
                
            for j in range(3-stacklen):
                result.append(nullidx)
    ...
    def get_output_representation(self, output_pair):  
        ...
        a=keras.utils.to_categorical(idx, num_classes=91) 
